...
'''
final_final_num = re.sub(r'(\d+)/(\d+)+/?(\d+)?',
                     '\1\2\3', nprop)
'''
final_final = re.sub(r'(?P<pal_lema>[a-z]+(-[a-z]+)*/[a-z]+(-[a-z]+)*)/\S*',
                     '\g<pal_lema>', nprop)


# Removendo palavras e pontuação para gerar apenas os lemas
# Removing words and punctuation to generate only the lemmas
so_lemas = re.sub(r'[a-z]+/(?P<lema>[a-z]+)', '\g<lema>', final_final)
# so_lemas = re.sub(r'(\d+)(/\d+)*/card', '\1', so_lemas)
