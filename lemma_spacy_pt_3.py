...
# Consertando duas palavras específicas
# Fixing two specific words
vou = re.sub(r'\b(?P<vou>vou)\S+', '\g<vou>/ir', pelos)
diga = re.sub(r'\b(?P<diga>diga)\S+', '\g<diga>/dizer', vou)
...
de_pron = re.sub(r'\bd(?P<rad>aquel|es?)(?P<var>[lts]?)(?P<des>[ea]s?)\b\S+',
                 'de/de \g<rad>\g<var>\g<des>/\g<rad>\g<var>e', diga)
...
print(se_se)
final_final = re.sub(r'(?P<pal_lema>[a-zA-Z]+(-[a-zA-Z]+)*/[a-zA-Z]+(-[a-zA-Z]+)*)/\S*',
                     '\g<pal_lema>', se_se)

# Removendo palavras e pontuação para gerar apenas os lemas
# Removing words and punctuation to generate only the lemmas
so_lemas = re.sub(r'[a-zA-Z]+/(?P<lema>[a-zA-Z]+)', '\g<lema>', final_final)
